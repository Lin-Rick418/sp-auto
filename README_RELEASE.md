...
F7 關閉時探針不再掃描一般掉落物；Mode 3 的王死亡拾取階段會自行開啟必要掃描。F7＋F8 同時開啟時，v2.23.0 會以每輪最多 24 件、約 2 ms 的預算增量處理地面道具，最多輸出 96 件最相關候選；目前拾取目標優先更新。掉落物生命週期、出口快取及背景狀態寫入會避免大量物件集中阻塞 Unity 主執行緒，因此道具增加主要影響完成一輪快取所需時間，不再等比例增加單幀耗時。

撿取前會先放開 WASD、Left Shift、Right Shift，至少經過一個完全無按鍵迴圈（預設 50 ms）再送出物品互動；指定拾取封包本身也會清空移動與技能鍵。已消失物品會同步由互動快取及輸出快取移除，避免角色長時間繞著殘留目標徘徊。

拾取準備開始後會在伺服器真正的互動範圍內保留 0.25 世界單位的退出遲滯；角色因最後一個移動封包或位置快照抖動而略微越過進入門檻時，不再重新走路及重算放鍵等待。

F8 控制選單新增「拾取品質」：可直接選擇 `Common`、`Rare`、`Unique` 或 `Legendary` 以上，選完立即套用並保存，不需要手動編輯 JSON。
...
`lock_mouse_to_monster` 會用背景 `WM_MOUSEMOVE` 更新遊戲內游標位置，不再移動或占用 Windows 實體游標。`left_click_after_mouse_lock` 預設為 `false`；改成 `true` 後，每次取得新怪物目標並第一次成功送出背景游標位置時，會再對遊戲視窗送出一次背景左鍵。背景操作不切換焦點，且 v2.23.0 會要求 Unity 在失焦或最小化時繼續更新；預覽、F8 暫停、F2 跟隨、拾取、角色死亡及怪物位於畫面外時不送出滑鼠訊息。
...
`job_type = 1` 時，F8 控制選單會顯示「召喚／Buff 檢查設定」。可分別選擇是否維持 `SummonSkeleton`、`SummonAbomination`、`SummonSkeletonMage`、`Invoker` 與 `Conjurer`，並指定右側 NumPad 0–9 的技能鍵；預設全部關閉。F8 一般導航或 F2 純跟隨開始前都會先檢查；已選項目缺少時會放開 WASD 與左右 Shift、每次只背景觸發一個技能，全部確認存在後才自動開始導航。召喚物依 `SummonDisplays_C` 判斷至少一隻，Buff 只依目前 `StatusDisplays_C` 判斷，不使用可用 Buff 目錄。
...
若遊戲仍載入舊版探針，而舊資料夾中的 `spiritvale_memory_state.json` 正在持續更新，Python 會暫時自動使用同一舊路徑；安裝 v2.23.0 發行包並重啟遊戲後會優先回到 LocalAppData。安裝程式會移除舊的開機載入探針，改裝 Loader 至 `BepInEx\plugins`，完整探針則放在 `BepInEx\ondemand`。也可用環境變數 `SPIRITVALE_BOT_IPC_DIR` 明確指定 IPC 資料夾。
