...
$outputDll = Join-Path $PSScriptRoot 'PickupBehaviorTests.dll'
& 'C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe' /nologo /noconfig /nostdlib /target:exe "/out:$outputDll" $refs (Join-Path $PSScriptRoot 'PickupBehaviorTests.cs')
if ($LASTEXITCODE -ne 0) { throw 'Behavior test compilation failed' }
Set-Content (Join-Path $PSScriptRoot 'PickupBehaviorTests.runtimeconfig.json') '{"runtimeOptions":{"tfm":"net6.0","framework":{"name":"Microsoft.NETCore.App","version":"6.0.0"}}}'
