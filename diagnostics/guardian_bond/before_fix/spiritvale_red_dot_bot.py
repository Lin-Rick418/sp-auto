...
def missing_summoner_checks(
    ...
    needs_buffs = any(item in SUMMONER_BUFF_IDS for item in enabled_ids)
    ...
    *,
    ...
    for check_id in missing:
        ...
        state.retry_after[check_id] = now + max(
            0, int(config.summoner_check_retry_delay_ms)
        ) / 1000
        state.settle_until = now + max(
            0, int(config.summoner_check_settle_ms)
        ) / 1000
        return True
...
def run_bot(
    ...
    *,
    ...
    def request_skill_key(key: str, target_summon: bool = False) -> bool:
        ...
        the probe sets the input DTO's UnitId (FastCast) to its primary summon
        so a target-required buff (e.g. Guardian Bond) actually lands instead of
        firing on nothing.
        """
