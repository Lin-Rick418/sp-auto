...
{
    [BepInPlugin("local.spiritvale.positionprobe", "SpiritVale Position Probe", "2.22.0")]
    public sealed class Plugin : BasePlugin
    {
        ...
        public override void Load()
        {
            ...
            Log.LogInfo(
                "Memory navigation probe v2.22.0 loaded on demand; auto_relogin="
    ...
    internal static class BackgroundMovementPatch
    {
        ...
        private static void FireSkillKeyHotkeys(
            object playerController, object inputs, long requestId,
        ...
        )
        {
            ...
            if (targetSummon && inputs != null)
            {
                ...
                if (summon != null && summonId > 0)
                {
                    ...
                    // Focusing a summon sets PlayerController.player.
                    _targetPlayer.SetValue(playerController, summon, null);
                    ClickHotkeys(playerController, hotkeys);
                    _log.LogInfo(
                        "Guardian-bond cast injected: unit=" + summonId
                        + " target_player="
                        + PlayerUpdatePatch.GetUnitObjectId(
                            _targetPlayer.GetValue(playerController, null))
                        + " key=" + normalized
                    );
                    return;
                }
                // No summon yet: fall through so a later retry can still fire.
            }
            ClickHotkeys(playerController, hotkeys);
        }
    ...
    internal static class PlayerUpdatePatch
    {
        ...
        // The summon UNIT OBJECT to cast a target-required buff (e.g. Guardian
        // Bond) on. Prefers Summoning.Primary (set only while mounted / with a
        // designated primary), then falls back to any of the player's own scene
        // summons. Returns null when no summon can be found. The caller focuses
        // this object (PlayerController.CastTarget) before firing the skill.
        internal static object ResolveSummonUnit(object playerController)
        {
            try
            {
                ...
                {
                    object summoning = _playerSummoning.GetValue(
                        playerController, null
                    );
                    if (summoning != null)
                    {
                        object primary = _summonPrimary.GetValue(
                            summoning, null
                        );
                        if (primary != null && GetUnitObjectId(primary) > 0)
                            return primary;
                    }
                }
            }
            ...
            {
                // Fall through to scene resolution.
            }
            return ResolveAnySummonUnit();
        }

        // The player's ordinary summons (skeletons, etc.) are not in
        // Summoning.Primary/ActiveSummons; they live in the scene as friendly
        // (non-enemy team) MonsterController units. Enumerate the same scene
        // source the monster scan uses and return the first living friendly
        // summon object, applying the scan's own liveness gates.
        internal static object ResolveAnySummonUnit()
        {
            ...
            try
            {
                object list = _findSceneMonsters.Invoke(
                    null, new object[] { _findObjectsSortNone }
                );
                if (list == null)
                    return null;
                ...
                for (int index = 0; index < count; index++)
                {
                    ...
                    if (Convert.ToBoolean(
                        _isTrainingDummy.GetValue(monster, null)))
                        continue;
                    if (!Convert.ToBoolean(
                        _isActive.GetValue(monster, null)))
                        continue;
                    if (!Convert.ToBoolean(
                        _isAliveAndDisplayed.GetValue(monster, null)))
                        continue;
                    ...
                    int team = Convert.ToInt32(
                        _monsterTeam.GetValue(data, null),
                        CultureInfo.InvariantCulture
                    );
                    // Non-enemy team == the player's own summons.
                    if (team == EnemyCombatTeamValue)
                        continue;
                    if (GetUnitObjectId(monster) > 0)
                        return monster;
                }
            }
            catch
